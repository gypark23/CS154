...
int addOK(int x, int y) {
  ...
  return !(~(sign_x ^ sign_y) & (sign_x ^ sign) & (sign_y ^ sign));
}
...
int isAsciiDigit(int x) {
  ...
  int checklow = x + (~lower);
  int checkup = (~x) + upper;

  return !((checklow | checkup) >> 31);
}
...
int isLess(int x, int y) {
  int substract = x + (~y + 1);
  int xysigns = x & (~y);
  int substsign = (~(x ^ y)) & substract;

  return ((xysigns | substsign) >> 31) & 1;
}
...
int rotateLeft(int x, int n) {
  ...
  return ((x >> (32 + substract)) & ~(((~0) >> n) << n)) | (x << n);
}
